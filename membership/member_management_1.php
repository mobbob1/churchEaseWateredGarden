    ...
    try {
        if (!empty($_POST['id'])) {
            ...
            $sql = "INSERT INTO members (
                    ...
                    member_type_id, parent_id, created_by
                ) VALUES (
                    :name, :first_name, :surname, :email, :gender, :profession,
                    :home_address, :work_address, :gps_number,
                    :contact_number_1, :contact_number_2,
            ...
            // Check if member type is 'organic' and create user account
            $stmt = $pdo->prepare("SELECT type_name FROM member_types WHERE id = ?");
            $stmt->execute([$member_type_id]);
            $member_type = $stmt->fetchColumn();
            ...
            if ($member_type != 'Child') {
                ...
                if (!$existing_user) {
                    ...
                    $sql = "INSERT INTO users (
                        member_id, username, email, password, full_name, 
                        role, phone, created_by, password_changed
                    ) VALUES (
                        :member_id, :username, :email, :password, :full_name,
                        'member', :phone, :created_by, 0
                    )";

                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([
                        'member_id' => $member_id,
                        'username' => $contact_number_1,
                        'email' => $email,
                    ...
                    $message = "Welcome to OutpouringCRM! Your account has been created.\n";
                    $message .= "Username: " . $contact_number_1 . "\n";
                    $message .= "Password: " . $password . "\n";
                    $message .= "Login at: " . APP_URL . "\n";
                    $message .= "Please change your password after login.";

                    // Initialize Mnotify with config values
                    $endPoint = 'https://api.mnotify.com/api/sms/quick';

                    // Format phone number for Ghana
                    $phone = preg_replace('/[^0-9]/', '', $contact_number_1);
                    if (strlen($phone) > 9) {
                        $phone = substr($phone, -9); // Get last 9 digits
                    }
                    $formattedPhone = '233' . $phone; // Add Ghana country code
                    ...
                    // Debug info
                    error_log('Debug - API Key: ' . MNOTIFY_API_KEY);
                    ...
                    try {
                        ...
                        $logger->log(
                                ...
                                $http_status == 200 ? 'success' : 'failed'
                        );
                    } catch (PDOException $e) {
...
<html lang="en">
    <head>
        ...
        <title><?php echo isset($member) ? 'Edit' : 'Add'; ?> Member - OutpouringCRM</title>
